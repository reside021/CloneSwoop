...
public class Plane_Move : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        radius = Mathf.Sqrt(Mathf.Pow(_plane.transform.position.x, 2) + Mathf.Pow(_plane.transform.position.y - 22f, 2) + Mathf.Pow(_plane.transform.position.z, 2));
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (_plane.transform.position.y > 35f)
        {
            _rigidbody.velocity = Vector3.down;
        }
        else
        {
            if(_plane.transform.position.y <= 27f)
            {
